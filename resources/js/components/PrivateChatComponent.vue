<template>
        ...
        <div class="container1 row" style="border-radius: 20px; overflow:hidden; box-shadow: -10px 10px 5px grey; position:absolute; margin-top: 40px; margin-left: 100px;">
            <div class="left1">
                <div class="top1">
                    ...
                    <span class="dropdown search1" style="float: left; margin-right: 10px;top: 13px;left: 20px" aria-hidden="true">

                        <div v-if="auth.name<20">{{ auth.name }}</div>
                        <div v-else>  {{auth.name.substring(0,20)}}</div>

                    </span>


                    <a  onclick="event.preventDefault();document.getElementById('logout-form').submit();"    style="float: right; font-size: 22px; padding: 10px;" >        <i class="fa fa-sign-out1"></i></a>
                ...
                <ul class="people1">
                    <li class="person1"    @click.prevent="openChat(friend)"  v-for="friend in friends" :key="friend.id">
                        <img :src="base_url+'public/AdminLTE/dist/img/avatar5.png'" alt="" />
                        ...
                        <span  class="preview1" style="color:#00b0ff" v-if="friend.online" >Online</span>
                        <span  class="preview1" v-else>Offline</span>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            getFriends() {
                axios.post(this.base_url+"getFriends").then(res => {
