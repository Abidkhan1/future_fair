...
<script>
    export default {
        ...
        methods: {
            getAllMessages() {
                axios
                    .post(APP_URL+`session/${this.friend.session.id}/chats`)
            ...
            send() {
                if (this.message) {
                    ...
                    axios
                        .post(APP_URL+`send/${this.friend.session.id}`, {
            ...
            clear() {
                axios.post(APP_URL+`session/${this.friend.session.id}/clear`).then(res => {
                    this.chats = [];
                })
            },
            block() {
                ...
                axios
                    .post(APP_URL+`session/${this.friend.session.id}/block`)
                    .then(res => (this.session.blocked_by = auth.id));
            },
            unblock() {
                this.session.block = false;
                axios.post(APP_URL+`session/${this.friend.session.id}/unblock`).then(
                    res => {
            ...
            read() {
                axios.post(APP_URL+`session/${this.friend.session.id}/read`);
            }
